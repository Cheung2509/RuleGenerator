...
class VGDLParser(object):
    ...
    @staticmethod     
    def playGame(self, game_str, map_str, headless = False, persist_movie = False, movie_dir = "./tmpl"):
        """ Parses the game and level map strings, and starts the game. """
        g = VGDLParser.parseGame(self, game_str)
    ...
    def parseGame(self, tree):
        ...
        if not isinstance(tree, Node):
            str = indentTreeParser(tree).children[0]
        print(str.content)
        sclass, args = self._parseArgs(str.content)
        self.game = sclass(**args)        
        for c in str.children:
    ...
    def _eval(self, estr):
        ...
        #Importing everything individually because it won't let me import all
        from vgdl.ontology import GREEN, RED, BLUE, GRAY, WHITE
        from vgdl.ontology import Immovable, MovingAvatar, FlakAvatar, Missile, AimedAvatar, OrientedAvatar, ShootAvatar
        from vgdl.ontology import Bomber, SpawnPoint, SpriteCounter, MultiSpriteCounter, stepBack, turnAround, killSprite, collectResource
        from vgdl.ontology import UP, DOWN, RIGHT, LEFT
        from vgdl.ontology import Fleeing, Chaser, RotatingFlippingAvatar, bounceForward, killIfFromAbove, pullWithIt, bounceDirection, reverseDirection
        from vgdl.ontology import flipDirection, windGust, conveySprite, wallBounce, killIfAlive, cloneSprite, Passive, attractGaze
        from vgdl.ontology import Timeout, Flicker,RandomNPC, transformTo, killIfOtherHasMore, Conveyor, RotatingAvatar, slipForward, undoAll
        from vgdl.ontology import killIfHasLess, killIfHasMore, wrapAround
        return eval(estr)
...
class BasicGame(object):
    ...
    def buildLevel(self, lstr):        
        ...
        assert minimum==maximum, "Inconsistent line lengths."
...
class VGDLSprite(object):
    ...
    def _updatePos(self, orientation, speed=None):
        if speed is None:
            speed = self.speed
        if not(self.cooldown > self.lastmove or abs(orientation[0] + orientation[1])==0):
